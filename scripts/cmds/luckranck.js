module.exports = {
  config: {
    ...
    guide: {
      ...
      en: "   {pn} <إسم الأمر> <القاعدة الجديدة>: قم بضبط القاعدة للأمر"
        ...
        + "\n    {pn} مستواي الأصل: إستعادة الأمر إلى ما كان عليه في البداية"
        + "\n—————"
  ...
  onStart: async function ({ message, event, args, role, threadsData, getLang }) {
    ...
    await message.reply("✅ " + (Default === true ? getLang("resetRole", commandName) : getLang("changedRole", commandName, newRole)));
  }
};
