...
module.exports = {
  ...
  config: {
    ...
    countDown: 5,
    role: 0,
    shortDescription: 'تحميل تلقائي للمقطاع عن طريق الروابط',
    longDescription: { en: 'تنزيل الفيديو تلقائيًا لـ Facebook وInstagram وTikTok وYouTube وTwitter وSpotify' },
    category: 'وسائط',
    guide: '{p}{n}'
  },
  ...
  downLoad: async function (url, api, event) {
    ...
    api.sendMessage("⏱️ | جاري معالجة طلبك يرجى الانتظار....", event.threadID, event.messageID);
    ...
    try {
      ...
      if (url.includes("facebook.com")) {
        ...
        return api.sendMessage({
          body: `• العنوان : ${metadata.title}\n• الألبوم : ${metadata.album}\n• الفنان : ${metadata.artists}\n• تاريخ الرفع : ${metadata.releaseDate}`,
      ...
      if (contentUrl) {
        ...
        api.sendMessage(" ❌ | حدث خطأ أثناء محاولة تنزبل المفطع.", event.threadID, event.messageID);
      }
    } catch (e) {
      console.error(e);
      api.sendMessage(" ❌ |عذرا، حدث خطأ أثناء معالجة طلبك.", event.threadID, event.messageID);
    }
  }
};
